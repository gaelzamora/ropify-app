...
// DeleteS3Folder elimina todos los objetos con el prefijo especificado (simulando eliminar una carpeta)
func DeleteS3Folder(prefix string) error {
    // Crear sesión de AWS
    sess, err := session.NewSession(&aws.Config{
        Region: aws.String(os.Getenv("AWS_REGION")),
    })
    ...
    // Listar todos los objetos con el prefijo dado
    ...
    err = svc.ListObjectsV2Pages(&s3.ListObjectsV2Input{
        ...
        return true // continuar paginando
    })
    ...
    // Si no hay objetos que eliminar, terminar
    if len(objectsToDelete) == 0 {
        return nil
    }

    // Eliminar todos los objetos en un solo lote
